...
fun typeof (e1, delta, gamma)  = 
       let fun literal (NUM x) = inttype
              | literal (BOOLV n) = booltype
              | literal (SYM n) = symtype
              | literal (NIL) = FORALL (["'a"],listtype(tyvarA))
              | literal (PAIR (v, NIL)) = listtype 
                                    (typeof (LITERAL v, delta, gamma))
              | literal (PAIR (v1, v2)) =
                    let val tau_v1 = typeof(LITERAL v1,delta,gamma)
                    in
                        if (eqType (tau_v1, asType(tau_v1, delta))) then
                            typeof (LITERAL v2, delta, gamma)
                        else
                            raise TypeError ("literal pair error")
              
                    end
              | literal (CLOSURE (v1, v2)) = raise TypeError ("closure error")
              | literal (PRIMITIVE v) = raise TypeError ("primitive error")
              | literal (ARRAY v) = raise LeftAsExercise "to do: array"
       fun ty (LITERAL x) = literal x
              | ty (VAR x) = find (x, gamma)
              | ty (SET (x, e)) =
                    let val tau1 = ty e
                    in
                        if eqType (find (x, gamma), tau1) then
                            tau1
                        else
                            raise TypeError ("set error")
                    end
              | ty (WHILEX (e1, e2)) =
                    let val tau1 = ty e1
                        val tau2 = ty e2
                    in
                        if eqType (tau1, booltype) then
                            ...
                            raise TypeError ("while e1 of type " ^ 
                                typeString tau1 ^ " and e2 of type " ^ 
                                typeString tau2)
                    end
              | ty (IFX (e1, e2, e3)) = 
                    let val tau1 = ty e1
                    ...
                    in 
                        if eqType (tau1, booltype) andalso 
                            ...
                            raise TypeError ("if e1 of type " ^ typeString tau1 
                                ^ " and e2 of type " ^ typeString tau2 
                                ^ " and e3 of type " ^ typeString tau3)
                    end
              | ty (BEGIN es) = 
                    let val tlist = map ty es
                    in
                        List.last tlist handle Empty => unittype
                    end
              | ty (LETX (LET, l, e)) =
                        let val xlist = map fst l
                            val elist = map snd l
                            val tlist = map ty elist
                        in
                            typeof (e, delta, bindList (xlist, tlist, gamma))
                        end      
              | ty (LETX (LETSTAR, [], e)) =
                    ty e
              | ty (LETX (LETSTAR, x::xs, e)) =
                    typeof (LETX (LET, [x], LETX (LETSTAR, xs, e)), delta, gamma)
              | ty (LETRECX (l, e)) = 
                    let val elist = map snd l
                        val xlist = map fst (map fst l)
                        val tlist = map snd (map fst l)
                        val new_tlist = foldr 
                                (fn (x,y) => (typeof (x,delta,bindList 
                                (xlist,tlist,gamma)))::y) [] elist
                    in
                        if (foldr 
                                (fn (y, x) => ((eqType ((asType (y, delta)), y))
                                 andalso x)) true tlist) 
                        andalso (eqTypes (new_tlist,tlist))  
                        andalso (foldr (fn (a, b) => 
                                (case a of LAMBDA (i, j) => b
                                    | _ => false)) true elist)
                        then typeof (e,delta,bindList (xlist,tlist,gamma))
                        else
                            raise TypeError("Letrec error")
                    end
              | ty (LAMBDA (l, e)) =
                    let val xlist = map fst l
                        val tlist = map snd l
                    in
                        if (foldr (fn (y, x) => ((eqType 
                                    ((asType (y, delta)), y)) 
                                    andalso x)) true tlist) then
                            FUNTY (tlist, typeof (e, delta, bindList 
                                    (xlist, tlist, gamma)))
                        else
                            raise TypeError ("lambda error")
                    end
              | ty (APPLY (e, es)) = 
                    let val tau1 = typeof (e, delta, gamma) 
                        val tau_list = case tau1 of FUNTY(tyexl,tyex) => tyexl
                                          | _ => raise TypeError("apply error")
                        val tau2 = case tau1 of FUNTY (tyexl, tyex) => tyex
                                      | _ => raise TypeError ("apply error") 
                    in 
                        if eqTypes (map ty es,tau_list) then
                            tau2
                        else 
                            raise TypeError ("apply error2")
                    end 
              | ty (TYLAMBDA (l, e)) =
                    let val starlist = (foldr (fn (x,y) => (TYPE::y)) [] l)
                    in
                        if (List.null (inter (l, freetyvarsGamma gamma))) then
                            FORALL (l, typeof(e, 
                                    (bindList(l,starlist,delta)), gamma))
                        else 
                            raise TypeError("tylambda error")
                    end
              | ty (TYAPPLY (e, l)) =
                    let val forall = typeof (e, delta, gamma)
                    in
                        instantiate (forall, l, delta)
                    end
        in
            ty e1
        end

fun elabdef (VAL (x, e), delta, gamma) = 
    let val tau_1 = typeof (e, delta, gamma)
    in 
        (bind (x, tau_1, gamma), typeString tau_1)
    end
  | elabdef (VALREC (x, t, e), delta, gamma) = 
    let val tau_1 = case e of LAMBDA (a, b) => 
                                typeof (e, delta, bind (x,t,gamma))
                       | _ => raise TypeError ("valrec error")
    in 
        if (eqType (tau_1, t)) 
        then 
            (bind (x, t, gamma), typeString tau_1)

        else
            raise TypeError ("valrec error")
    end
  | elabdef (EXP e, delta, gamma) = elabdef ((VAL ("it", e)), delta, gamma)
  | elabdef (DEFINE (f, t, le), delta, gamma) = 
    let val tlist = map snd (fst le)
        val l = fst le
        val e = snd le
    in
      elabdef ((VALREC (f, FUNTY (tlist, t), LAMBDA (l, e))), delta, gamma)
    end
      ...
      val (types, values) = foldl addPrim (emptyEnv, emptyEnv)
                           ...
                           (* primitive functions for \tuscheme\ [[::]] 1264a *)
                             ...
                             ("cdr",  unaryOp  (fn (PAIR (_, cdr)) => cdr 
                                   ...
                                   , FORALL (["'a"], FUNTY ([listtype tyvarA],
                                                           listtype tyvarA))) ::
                            ("Array.make", binaryOp (fn (NUM n, v) => if (n > 0) then  makeArray(n,v)
                                                                      else raise RuntimeError ("array make error")
                                                        | _ => raise RuntimeError ("array make error"))
                                    , FORALL (["'a"], FUNTY ([inttype, tyvarA], 
                                                        arraytype tyvarA))) ::
                            ("Array.length", unaryOp (fn (ARRAY a) => arrayLength a
                                                         | _ => raise RuntimeError ("array length error"))
                                    , FORALL (["'a"], FUNTY ([arraytype tyvarA], inttype))) ::
                            ("Array.at", binaryOp (fn (ARRAY a, NUM n) => arrayAt(a, n)
                                                        | _ => raise RuntimeError ("array at error"))
                                    , FORALL (["'a"], FUNTY ([arraytype tyvarA, inttype], tyvarA))) ::
                            ("Array.at-put", (fn [ARRAY a, NUM i, v] => (arrayAtPut(a, i, v); unitVal)
                                                        | _ => raise RuntimeError ("array at put error"))
                                    , FORALL (["'a"], FUNTY ([arraytype tyvarA, inttype, tyvarA], unittype))) ::
