...
(* function [[ty]], to check the type of an expression, given $\itenvs$ ((prototype)) 417 *)
      | ty (AAT (a, i))        = 
            let val tau_a = case ty a of ARRAYTY tau => tau
                               | _ => raise TypeError ("Array of invalid type")
                val tau_i = ty i
            in if eqType (tau_i, INTTY) then
                tau_a
                else
                    raise TypeError ("Array of type " ^ typeString tau_a ^ " at i of type " ^ typeString tau_i)
            end
      | ty (APUT (a, i, e))    =
            let val tau_a = case ty a of ARRAYTY tau => tau
                               | _ => raise TypeError ("Array of invalid type")
                val tau_i = ty i
                val tau_e = ty e
            in if eqType (tau_i, INTTY) andalso eqType (tau_e, tau_a) then
                tau_e
                else
                    raise TypeError ("Put element of type " ^ typeString tau_e ^ " at index of type " ^ typeString tau_i ^ "in array of type " ^ typeString tau_a)
            end
      | ty (AMAKE (len, init)) =
            let val tau_l = ty len
                val tau_i = ty init
            in if eqType (tau_l, INTTY) then
                ARRAYTY tau_i
                else
                    raise TypeError ("Make array of type " ^ typeString tau_i ^ "of length of type " ^ typeString tau_l)
            end
      | ty (ASIZE a)           = 
            let val tau_a = case ty a of ARRAYTY tau => INTTY
                               | _ => raise TypeError ("Array of invalid type")
            in tau_a end
